...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const productCheck = await query('SELECT id, name, sku FROM products WHERE id=$1', [productId]);
    ...
    if (alertCount > 0) {
      await query('DELETE FROM low_stock_alerts WHERE product_id = $1', [productId]);
