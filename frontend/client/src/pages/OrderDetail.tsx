...
interface OrderItem {
  ...
  product_sku: string;
  quantity: number;
  unit_price: string; // Comes as string from database
}
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-lg font-semibold text-primary">${order.total_amount || '0.00'}</p>
            </CardContent>
          </Card>
        </div>
                  ...
                  <tbody>
                    {order.items.map((item) => (
                      <tr key={item.id} className="border-b border-border hover:bg-orange-50">
                        ...
                        <td className="py-3 px-4 text-right text-foreground">${parseFloat(item.unit_price).toFixed(2)}</td>
