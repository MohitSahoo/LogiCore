...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Set up periodic role check (every 10 seconds for faster updates)
  ...
  const refreshUserProfile = async () => {
    try {
      ...
      if (user && freshUser.role !== user.role) {
        console.log(`🔄 User role changed from ${user.role} to ${freshUser.role}`);
        setUser(freshUser);
        
        // Show notification about role change
        const roleChangeMessage = freshUser.role === 'admin' 
          ? '🎉 You have been promoted to Admin!' 
          : '📋 Your role has been changed to User';
        
        // Use a simple alert since we don't have toast in this context
        setTimeout(() => {
          alert(roleChangeMessage + '\n\nThe page will reload to apply changes.');
